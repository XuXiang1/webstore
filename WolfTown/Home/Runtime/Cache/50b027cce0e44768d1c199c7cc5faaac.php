<?php if (!defined('THINK_PATH')) exit();?>﻿<!DOCTYPE HTML><html><head><title>狼堡-
		...
		function startPaging(page_index, flag) {
			...
			var postdata = {
				...
				d1: <?php if(empty($parentid)): ?>0 <?php else: echo ($parentid); endif; ?> ,
				d2: <?php if(empty($childid)): ?>0 <?php else: echo ($childid); endif; ?> ,
				d3: <?php if(empty($brandid)): ?>0 <?php else: echo ($brandid); endif; ?> ,
			};
			$("#myProductList").html("");
			...
			$.ajax({
				...
				</script><div class="container"><div class="women_main"><!-- start sidebar --><div class="col-md-3 s-d" style="padding-left: 2px;"><div class="w_sidebar"><section class="sky-form"><h4>全部产品</h4><div class="row1 scroll-pane"><div class="col col-4"><?php if(is_array($parentdata)): $j = 0; $__LIST__ = $parentdata;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$datap): $mod = ($j % 2 );++$j;?><label class="radio"><?php if(empty($parentid)): if($j == 1): ?><input type="radio" name="parent" checked="checked" pdata="<?php echo ($datap[ID]); ?>" onclick="changeclass(this)"><i></i><?php echo ($datap[classname]); else: ?><input type="radio" name="parent" pdata="<?php echo ($datap[ID]); ?>" onclick="changeclass(this)"><i></i><?php echo ($datap[classname]); endif; else: if($parentid == $datap[ID]): ?><input type="radio" name="parent" checked="checked" pdata="<?php echo ($datap[ID]); ?>" onclick="changeclass(this)"><i></i><?php echo ($datap[classname]); else: ?><input type="radio" name="parent" pdata="<?php echo ($datap[ID]); ?>" onclick="changeclass(this)"><i></i><?php echo ($datap[classname]); endif; endif; ?></label><?php endforeach; endif; else: echo "" ;endif; ?></div></div></section><section class="sky-form"><h4>类别</h4><div class="row2 scroll-pane"><div class="col col-4"><?php if(is_array($childdata)): $k = 0; $__LIST__ = $childdata;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$datac): $mod = ($k % 2 );++$k;?><label class="radio"><?php if(empty($childid)): if($k == 1): ?><input type="radio" name="child" checked="checked" pdata="<?php echo ($datac[ID]); ?>" onclick="changeclass(this)"><i></i><?php echo ($datac[classname]); else: ?><input type="radio" name="child" pdata="<?php echo ($datac[ID]); ?>" onclick="changeclass(this)"><i></i><?php echo ($datac[classname]); endif; else: if($childid == $datac[ID]): ?><input type="radio" name="child" checked="checked" pdata="<?php echo ($datac[ID]); ?>" onclick="changeclass(this)"><i></i><?php echo ($datac[classname]); else: ?><input type="radio" name="child" pdata="<?php echo ($datac[classid]); ?>" onclick="changeclass(this)"><i></i><?php echo ($datac[classname]); endif; endif; ?></label><?php endforeach; endif; else: echo "" ;endif; ?></div></div></section><section class="sky-form"><h4>品牌</h4><div class="row3 scroll-pane"><div class="col col-4"><?php if(is_array($branddata)): $l = 0; $__LIST__ = $branddata;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$datab): $mod = ($l % 2 );++$l;?><label class="radio"><?php if(empty($brandid)): if($l == 1): ?><input type="radio" name="brand" checked="checked" pdata="<?php echo ($datab[ID]); ?>" onclick="changeclass(this)"><i></i><?php echo ($datab[name]); else: ?><input type="radio" name="brand" pdata="<?php echo ($datab[ID]); ?>" onclick="changeclass(this)"><i></i><?php echo ($datab[name]); endif; else: if($brandid == $datab[ID]): ?><input type="radio" name="brand" checked="checked" pdata="<?php echo ($datab[ID]); ?>" onclick="changeclass(this)"><i></i><?php echo ($datab[name]); else: ?><input type="radio" name="brand" pdata="<?php echo ($datab[ID]); ?>" onclick="changeclass(this)"><i></i><?php echo ($datab[name]); endif; endif; ?></label><?php endforeach; endif; else: echo "" ;endif; ?></div></div></section></div></div><!-- start content --><div class="col-md-9 w_content gray-div"><div class="women"><a href="#"><h4><span>共 <span id="mytempcount">1</span> 件商品</span></h4></a><div class="clearfix"></div></div><!-- end grids_of_4 --><div style="display: block; width: 100%; text-align: center;" id="loadingImage"></div><!-- grids_of_4 --><div class="grids_of_4 row" id="myProductList"></div><div id="Pagination" class="m-pagination" style="padding-left: 15px;"></div></div><div class="clearfix"></div><!-- end content --></div></div><div class="foot-top"><div class="container"><div class="col-md-6 s-c"><li><div class="fooll"><h5>成为我们的加盟商！</h5></div></li><div class="clearfix"></div></div><div class="col-md-6 s-c"><div class="stay"><form action="__ROOT__/contactus/sendmessage" method="post"><div class="stay-left"><input type="text" name="contact-email" placeholder="输入您的邮箱地址" required=""><input type="hidden" name="contact-message" value="我要加盟" /><input name="Name" maxlength="100" type="hidden" placeholder="" value="匿名"></div><div class="btn-1"><input type="submit" value="加盟"></div></form><div class="clearfix"></div></div></div><div class="clearfix"></div></div></div><div class="footer"><div class="container"><div class="col-md-2 cust"><h4>用户指南</h4><li><a href="#">帮助中心</a></li><li><a href="#">FAQ</a></li><li><a href="buy.html">如何购买</a></li><li><a href="#">物流</a></li></div><div class="col-md-2 cust"><h4>关于我们</h4><li><a href="#">公司发展历程</a></li><li><a href="#">核心竞争力</a></li><li><a href="#">联系我们</a></li></div><div class="col-md-2 myac"><h4>友情链接</h4><li><a href="#">链接1</a></li><li><a href="#">链接2</a></li><li><a href="#">链接3</a></li><li><a href="#">链接4</a></li></div><div class="col-md-3 myac"><h4>狼堡店铺</h4><li><a href="#">上海松江狼堡品牌折扣馆</a></li><li><a href="#">上海松江狼堡品牌折扣馆</a></li><li><a href="#">杭州萧山狼堡品牌工厂店</a></li><li><a href="#">上海金山狼堡·身外织物时尚生活体验馆</a></li><li><a href="#">贵州安顺狼堡品牌集合店</a></li></div><div class="col-md-3 our-st"><h4>联系方式</h4><li><i class="add"></i>贵州安顺狼堡品牌集合店</li><li><i class="phone"></i>025-8888888</li><li><a href="mailto:info@example.com"><i class="mail"></i>info@sitename.com </a></li></div><div class="clearfix"></div><p>Copyright &copy; 2015.Company name All rights reserved. </p></div></div></body></html>